...
### 7.2. Разные денежные типы

В интерфейсах Delphi суммы передаются как `Currency`. В `outlay.summ` миграция
использует `DECIMAL(7,2)`, поэтому расходы рассчитаны на копейки. Но
`money_from_parents.summ_to_first_november` в текущей миграции объявлено как
`INT`. Значит, база платежей не описывает дробные суммы так же хорошо, как код
Delphi. Это ограничение схемы, которое стоит исправить отдельно, если платежи
должны хранить копейки.
...
3. **Тип суммы платежа.** Код Delphi использует `Currency`, но колонка
   `money_from_parents.summ_to_first_november` объявлена как `INT`. Дробная часть
   платежа не имеет подходящего места хранения.
