...
7. `code_explorer/files/<модуль>.js` — синхронизировать учебную копию кода
   (строка `code:`) и диапазоны строк в `annotations`, иначе просмотрщик
   покажет устаревший код.
...
8. Если создан новый `.pas`-модуль с UI — добавить его копию в
   `code_explorer/files/` и в список `ORDER` в `app.js`.
...
## Утилита dbquery (чтение БД из консоли)

`dbquery.ps1` + обёртка `dbquery.cmd` — быстрый просмотр данных SQL Server
агентом без запуска основного приложения (PowerShell + ADO.NET, сборка не нужна):

```powershell
& .\dbquery.cmd -q "SELECT TOP 5 * FROM outlay ORDER BY id DESC"
& .\dbquery.cmd -q "SELECT id, children_name FROM parents_and_children" -csv -top 20
& .\dbquery.cmd -f script.sql -db foura_tests
```

- По умолчанию **только чтение**: разрешены пакеты, начинающиеся с
  `SELECT/WITH/DECLARE/USE/SET/PRINT`; всё выполняется в транзакции
  с `ROLLBACK`. Пакеты с другими первыми словами (включая `EXEC`) отклоняются
  с кодом `2`.
- Границы защиты: проверка по первому слову не гарантия (`DECLARE ...; UPDATE`
  пройдёт), реальный барьер — ROLLBACK. Он откатывает любые DML и DDL внутри
  локальной базы, но не внешние эффекты (linked server/OPENROWSET,
  `sp_configure`, `xp_cmdshell`) и счётчики IDENTITY/sequence.
- Изменения данных — только с ключом `-Rw` (автокоммит, без транзакции).
  Использовать осознанно: по умолчанию цель — боевая база `foura`.
- Параметры: `-server`, `-db`, `-top N` (0 = без ограничения), `-timeout`,
  `-csv`, `-rw`, запрос также можно передать через stdin или файл `-f`
  (пакеты разделяются строкой `GO`: регистр и отступы не важны,
  `GO N` повторяет батч N раз; ограничение как у sqlcmd — строка `GO`
  внутри многострочного строкового литерала примется за разделитель).
- Многострочный `-q` передавайте напрямую в `dbquery.ps1` или через
  stdin/файл: обёртка `.cmd` ломает переносы строк в аргументах.
- Коды возврата: `0` — успех, `1` — ошибка выполнения, `2` — ошибка
  использования или запрет записи. Служебные сообщения идут в stderr,
  данные — в stdout, поэтому пайп в файл даёт чистый CSV.
- Файл `dbquery.ps1` — UTF-8 с BOM и CRLF (как `.pas`), иначе PowerShell 5.1
  ломает кириллицу в справке.
...
| `dbquery.ps1` / `dbquery.cmd` | CLI-утилита для чтения БД агентом (SQL → таблица/CSV) |
