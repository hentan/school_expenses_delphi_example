  ...
  annotations: [
    {
      ...
      explanation: 'БАЗОВАЯ форма редактирования — родитель для форм платежа и расхода. Это визуальное наследование форм (form inheritance): компоненты и их раскладка объявляются в .dfm базовой формы, наследники переопределяют виртуальные методы. Шаблонный метод (template method pattern): базовый класс задаёт скелет (3 Edit + 2 Button + валидация), наследники настраивают детали. uUpdatePupilForm НЕ наследник этой формы — у него своя структура.'
    },
    {
    ...
    },
    {
      ...
      explanation: 'reintroduce — ключевое слово Delphi: «я намеренно скрываю унаследованный метод с другой сигнатурой». У TForm уже есть конструктор Create(AOwner), а здесь объявлен свой Create с другими параметрами (Adate, Apurpose, ASum). Без reintroduce компилятор выдал бы предупреждение о сокрытии; с ним — предупреждение исчезает, потому что вы явно сказали: «да, я знаю и хочу так».'
    },
    {
