...
`,
  annotations: [
    ...
    },
    {
      ...
      explanation: 'Расходы класса. id — IDENTITY(1,1), автоинкрементный первичный ключ (в отличие от pupils, где id задаётся вручную). date_purchaise (дата покупки, DATE), item_name (на что потрачено), customer (кто платил — id ученика), summ DECIMAL(7,2) — сумма с 2 знаками после запятой, копейки не теряются. NULL допустим (расход может быть без суммы?).'
    },
    {
    ...
    },
    {
      ...
      explanation: 'AFTER DELETE для расходов: копирует удалённую строку в outlay_arc. Архив расходов хранит только удаления (обновлений расходов в приложении нет — расходы редактируются через delete+insert? нет, Update есть, но архивного триггера на update для outlay нет — только на delete).'
    },
    {
